...
CreateRadialPlot <- function(plot.data,
                             ...
                             legend.labels = rownames(plot.data)) {
  ...
  # ... + group (cluster) 'paths' HERE THE DATA GETS PLOTTED
  ncolours <- nrow(plot.data)
  colours <- brewer.pal(ncolours, "YlOrRd")
  ...
  if (plot.legend==TRUE) base  <- base + 
    ...
    scale_colour_brewer(labels=legend.labels, palette="YlOrRd")
